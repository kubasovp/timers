...
| Implementation structure | `docs/implementation/development-view.md` | Слои, импорты, DoD модулей |
...
## 2) Статус legacy для Functional Specification

`docs/Functional-Specification-Document.md` получает статус: **Legacy / Reference**.

Политика:
- не используется как source of truth для новых контрактов;
- может использоваться как исторический контекст;
- при конфликте приоритет у документов из source-of-truth matrix.

## 3) When to update (триггеры)
...
## 7) GRASP в документации — как использовать
