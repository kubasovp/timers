...
- `skip` пропускает текущий этап и фиксируется отдельным событием этапа (не как completed focus-фаза).
...
- Трей: Windows в MVP; macOS post-MVP; Linux tray не является частью MVP-корректности.
