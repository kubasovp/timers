...
- Пользовательский список custom timers не является history of runs. Completed timer-карточка — reusable entry, которую можно повторно запустить; при повторном запуске та же запись переходит из `completed` в `running`.
- Если пользователь останавливает повторно запущенную reusable timer-карточку до естественного завершения, карточка возвращается в `completed`. Новый таймер, который ни разу не завершался, при `stop` получает transient result `stopped`, исчезает из пользовательского списка и не удерживается как session-запись в storage.
- Удаление completed timer-карточки: hard-delete соответствующей timer session записи.
- `history_events` остаётся внутренним event log для recovery/debug/audit и не показывается как обязательный пользовательский список запусков в MVP.
